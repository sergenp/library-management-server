...
UPLOAD_FOLDER = os.path.join(*config.get("file").get("path"))
...
class Book(Resource):
    ...
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('name', type=str, help="name field is required")
        parser.add_argument('author_id', type=int, help="author_id field is required")
        parser.add_argument('category_id', type=int, help="category_id field is required")
        parser.add_argument('published_date', type=str, help="published_date field is required")
        parser.add_argument('book_cover', type=werkzeug.datastructures.FileStorage, location='files', help="You must provide a book_cover, allowed extensions are {','.join(ALLOWED_IMAGE_EXTENSIONS)}")
        data = parser.parse_args()
        if allowed_image(data['book_cover'].filename):
            filename = f"{uuid.uuid1()}.{data['book_cover'].filename.split('.')[1]}" 
            filepath = os.path.join(UPLOAD_FOLDER, filename)
            data['book_cover'].save(filepath)
        else:
        ...
        new_book = BookModel(name=data['name'], published_date=published_date, author=author, category=category, book_cover=filepath)
